...
To establish continuous integration/continuous delivery, you must implement one of two eventing models: the link:/docs/ref/general/reference/event-operator.html[events mediator] (which requires only a single, organization-level webhook), or individual webhooks. For more information and instructions on how to set either of these models up, see link:/docs/ref/general//configuration/tekton-webhooks.html[Connecting to GitHub with webhooks].
