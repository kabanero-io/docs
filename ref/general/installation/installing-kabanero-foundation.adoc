...
Kabanero version 0.8 has been tested on OpenShift Container Platform (OCP) 4.3. There is intent to expand testing to additional distributions, including upstream Kubernetes, in the future.
...
* `curl -s -O -L https://github.com/kabanero-io/kabanero-operator/releases/download/0.8.0/install.sh`
...
. Install the OLM CatalogSource containing the product operator. The following YAML can be used, after substituting the required version (the example uses version 0.8.0):
+
[source,yaml]
----
...
spec:
  sourceType: grpc
  image: kabanero/kabanero-operator-registry:0.8.0
----
...
You can build and deploy a simple java-openliberty application using the default java-openliberty pipelines by following these steps:
