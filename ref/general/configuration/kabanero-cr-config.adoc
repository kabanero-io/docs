...
A `Kabanero` custom resource (CR) instance describes a particular product
...
See the link:#examples[examples]. To define a product instance, you can
specify the following fields:

* `Spec`
** `version` - The release of the product that you want to install.
...
***** `url` - The URL pointing to the pipelines.  Typically this file
       ...
       HTTPS.
***** `skipCertVerification` - Controls whether the product controller will
...
** `landing`
*** `enable` - Controls whether the product landing page is deployed by
...
** `kabaneroInstance` - The overall status information of the
   product instance.
*** `ready` - The overall Status of the product.  A value of `True`
    indicates the product has been installed successfully.  A value of `False`
...
*** `version` - Shows the version of the product that is currently installed.
...
*** `hostnames` - Provides the hostnames from the `Route` that was created
    for the stack management CLI service.  The hostname can be used in the product
    CLI to connect to this product instance.
** `landing` - Contains information about the product landing page
*** `ready` - The overall status of the product landing page.
...
This example yaml defines a product instance at version 0.7.0, using
the default stacks.
