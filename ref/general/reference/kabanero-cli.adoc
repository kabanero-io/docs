...
Note: an `unknown` digest value when using a private container registry may indicate that a kubernetes secret needs to be created to authenticate to the the registry, see link:https://github.com/kabanero-io/docs/blob/davco01a-patch-1/ref/general/configuration/stack-governance.adoc#the-governance-policy-is-specified-on-the-kabanero-cr-instance[See Step 2, window=_blank].
...
----
kabanero cli version: 0.8.0
kabanero command line service image: kabanero/kabanero-command-line-services:0.8.0
----
