...
Use the following commands to monitor and administer the product environment.

=== Login

Use the stack management CLI URL to log in. There are two ways you can get this URL:

. View the CLI URL in the product landing page instance details.
...
Log in to a product instance by using GitHub credentials and store a temporary access token for subsequent command-line calls.
...
Run the `kabanero onboard` command to onboard a developer to the product environment.
...
=== Show the active repository application stack for a product instance

Make sure the product instance is installed in the Kabanero namespace. There are two ways to view the active application stacks repository for a product instance.
...
Run the `kabanero version` command to display the version of the product that is running.
...
Running the deactivate command removes the specified application stack from the list of available application types, without deleting it from the product instance.
...
Run the `kabanero logout` command to disconnect from the product instance.
