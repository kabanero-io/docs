...
The Valid Digest section can have four differest outputs:

matched: when the stack's digest from the Kabanero CR matches the container registry digest 

mismatched: when the stack's digest from the Kabanero CR does not match the container registry digest

NA: when a stack is deactivated it does not have a digest to compare

unknown: cannot read from the container registry, possible network timeout to the container registry server
