...
kabanero login <SM_CLI_URL> -u <GITHUB_USER_ID> --certificate-authority "mycert.cert"
...
* `mycert.cert` is a file containing a TLS certificate
...
Note: if you do use either the `--certificate-authority` or `--insecure-skip-tls-verify=true` options, you will be prompted to confirm that want to proceed with an insecure connection:
----
