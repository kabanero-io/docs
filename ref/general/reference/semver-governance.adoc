...
4. Update the Kabanero CR that is shared with developers with the new index that includes the latest patch level. Do not do this
step before the stack is activated, otherwise developers could create and push an application using the newer stack before it is
available.
5. Build pipelines label the containers created for the microservice with the identity of the three-digit application stack
...
6. For each updated container, rebuild, test, and redeploy the applications on the new patch level.

When retiring an older patch level of an application stack container:

1. For each container with an older patch level, rebuild, test, and redeploy the applications on the new patch level.
2. Remove application containers built on the older patch level from the container registry.
3. Remove the old index from the stack hub index shared with developers.
4. Deactivate the older patch level of the application (be sure to refresh and synchronize also).
